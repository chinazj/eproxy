...
const (
	...
	// LibraryPath is the default path to the eproxy libraries directory
	LibraryPath = "/var/lib/eproxy"

	// SockPath is the path to the UNIX domain socket exposing the API to clients locally
	SockPath = RuntimePath + "/eproxy.sock"

	// SockPathEnv is the environment variable to overwrite SockPath
	SockPathEnv = "eproxy_SOCK"
	...
	PidFilePath = RuntimePath + "/eproxy.pid"
	...
	BPFFSRootFallback = "/run/eproxy/bpffs"

	// TCGlobalsPath is the default prefix for all BPF maps.
	TCGlobalsPath = "tc/globals"

	// DefaultCgroupRoot is the default path where eproxy cgroup2 should be mounted
	DefaultCgroupRoot = "/run/eproxy/cgroupv2"
	...
	// which are not part of eproxy manged pods.
	EncryptNode = false
	...
	// ClientConnectTimeout is the time the eproxy-agent client is
	...
	// K8sWatcherEndpointSelector specifies the k8s endpoints that eproxy
	// should watch for.
	...
	// eproxyNode.Spec.IPAM.PreAllocate if no value is set
	IPAMPreAllocation = 8
	...
	ENIGarbageCollectionTagClusterName = "io.eproxy/cluster-name"
	...
	// AutoCreateeproxyNodeResource enables automatic creation of a
	// eproxyNode resource for the local node
	AutoCreateeproxyNodeResource = true
	...
	// which denotes the value of eproxy's K8s service cache size.
	...
	// RestoreV4Addr is used as match for eproxy_host v4 address
	RestoreV4Addr = "eproxy.v4.internal.raw "

	// RestoreV6Addr is used as match for eproxy_host v6 (router) address
	RestoreV6Addr = "eproxy.v6.internal.raw "
	...
	// InstallNoConntrackRules instructs eproxy to install Iptables rules to skip netfilter connection tracking on all pod traffic.
	...
	// EnableICMPRules enables ICMP-based rule support for eproxy Network Policies.
	...
	// Use the eproxyInternalIPs (vs. NodeInternalIPs) for IPsec encapsulation.
	UseeproxyInternalIPForIPsec = false
...
var (
	...
	BPFEventBufferConfigs = map[string]string{
		"eproxy_lxc": "enabled,128,0",
		// eproxy_ipcache is the likely the most useful use of this feature, but also has
		// the highest churn.
		"eproxy_ipcache":           "enabled,1024,0",
		"eproxy_tunnel_map":        "enabled,128,0",
		"eproxy_lb_affinity_match": "enabled,128,0",

		// ip4
		"eproxy_lb4_services_v2":    "enabled,128,0",
		"eproxy_lb4_backends_v2":    "enabled,128,0",
		"eproxy_lb4_reverse_nat":    "enabled,128,0",
		"eproxy_lb4_backends_v3":    "enabled,128,0",
		"eproxy_lb4_source_range":   "enabled,128,0",
		"eproxy_lb4_affinity_match": "enabled,128,0",

		// ip6
		"eproxy_lb6_services_v2":    "enabled,128,0",
		"eproxy_lb6_backends_v2":    "enabled,128,0",
		"eproxy_lb6_reverse_nat":    "enabled,128,0",
		"eproxy_lb6_backends_v3":    "enabled,128,0",
		"eproxy_lb6_source_range":   "enabled,128,0",
		"eproxy_lb6_affinity_match": "enabled,128,0",
	}
